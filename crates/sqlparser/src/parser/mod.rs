...
impl<'a> Parser<'a> {
    ...
    pub fn parse_prefix(&mut self) -> Result<Expr, ParserError> {
        ...
        return_ok_if_some!(self.maybe_parse(|parser| {
            match parser.parse_data_type()? {
                ...
                // db9 patch: Allow custom typed-string literals for PostgreSQL parity.
                // PostgreSQL accepts `mood 'happy'` for any user-defined type.
                // Only non-reserved keywords (IDENTITY, KEY, TYPE, VALUE, etc.)
                // are valid unquoted UDT names. Reserved keywords (FROM, WHERE,
                // SELECT, etc.) must be rejected — otherwise `SELECT FROM 'x'`
                // would parse as a typed-string instead of a syntax error.
                //
                // List from PostgreSQL 17 docs, Appendix C:
                // https://www.postgresql.org/docs/17/sql-keywords-appendix.html
                DataType::Custom(ref name, ref modifiers) => {
                    let is_reserved = name.0.first().map_or(true, |ident| {
                        ident.quote_style.is_none()
                            && is_pg_reserved_keyword(
                                &ident.value.to_uppercase(),
                            )
                    });
                    if is_reserved {
                        parser_err!("dummy", loc)
                    } else {
                        // Accept only actual string literal tokens — NOT bare
                        // identifiers. parse_literal_string() is too permissive
                        // (accepts Word(NoKeyword)), which would steal implicit
                        // aliases: `SELECT mood happy` must NOT parse as
                        // TypedString(mood, "happy").
                        let next = parser.next_token();
                        let value = match next.token {
                            Token::SingleQuotedString(s) => s,
                            Token::EscapedStringLiteral(s) => s,
                            Token::DollarQuotedString(s) => s.value,
                            _ => {
                                return parser.expected("string literal", next);
                            }
                        };
                        Ok(Expr::TypedString {
                ...
                // PostgreSQL rejects type[] 'string' for array types —
                // users must use '...'::type[] or CAST.
                DataType::Array(..) => parser_err!("dummy", loc),
                // db9 patch: use strict token matching instead of
                // parse_literal_string() which accepts bare identifiers
                // (Word(NoKeyword)), causing silent misparses like
                // `DATE U&'2024-01-01'` → `(DATE 'U') & '2024-01-01'`.
                data_type => {
                    let next = parser.next_token();
                    let value = match next.token {
                        Token::SingleQuotedString(s) => s,
                    ...
                    Ok(Expr::TypedString { data_type, value })
                }
            }
        }));
    ...
    pub fn parse_optional_column_option(&mut self) -> Result<Option<ColumnOption>, ParserError> {
        if self.parse_keywords(&[Keyword::CHARACTER, Keyword::SET]) {
            ...
            let characteristics = self.parse_optional_constraint_characteristics()?;
    ...
    /// Parse the optional constraint characteristics that may trail a
    /// `UNIQUE` / `PRIMARY KEY` / `FOREIGN KEY` constraint in PostgreSQL:
    /// `[ NOT ] DEFERRABLE [ INITIALLY { DEFERRED | IMMEDIATE } ]`.
    ///
    /// The two clauses may appear in either order and either may be omitted.
    ...
    pub fn parse_optional_constraint_characteristics(
        ...
        loop {
            if deferrable.is_none() && self.parse_keywords(&[Keyword::NOT, Keyword::DEFERRABLE]) {
                deferrable = Some(false);
            } else if deferrable.is_none() && self.parse_keyword(Keyword::DEFERRABLE) {
                ...
                if self.parse_keyword(Keyword::DEFERRED) {
                    ...
                    return self.expected("one of DEFERRED or IMMEDIATE", self.peek_token());
                }
            } else {
                break;
            }
        }
        if deferrable.is_none() && initially.is_none() {
            Ok(None)
        } else {
...
/// PostgreSQL 17 reserved keywords (Appendix C, "reserved" column).
/// These cannot be used as unquoted type names in typed-string literals.
/// Non-reserved keywords (IDENTITY, KEY, TYPE, VALUE, etc.) are allowed
/// because PostgreSQL permits them as unquoted identifiers / type names.
fn is_pg_reserved_keyword(upper: &str) -> bool {
    matches!(
        upper,
        "ALL"
            | "ANALYSE"
