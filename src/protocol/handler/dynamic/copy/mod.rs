...
) -> PgWireResult<(R, crate::session_context::TxnDirtyTableIds)> {
    crate::sql::query_context::with_scoped_query_context(
        qctx,
        crate::sql::runtime_context::wrap_with_statement_runtime_context(runtime, async move {
            let result = future.await;
            let dirty_tables = crate::session_context::current_statement_dirty_table_ids();
            result.map(|value| (value, dirty_tables))
        }),
    )
    .await
}
...
impl CopyHandler for DynamicPgHandler {
        ...
        PgWireError: From<<C as Sink<PgWireBackendMessage>>::Error>,
    {
        ...
        let mut statement_dirty_tables = crate::session_context::TxnDirtyTableIds::new();
        ...
        while offset < rows_to_insert.len() {
            ...
            let insert_res = with_copy_statement_context(&qctx, &runtime, async {
                let mut session = self.auth().session.lock().await;
            ...
            let chunk_dirty_tables = match insert_res {
                Ok(((), dirty_tables)) => dirty_tables,
                Err(e) => {
                    let mut session = self.auth().session.lock().await;
                    rollback_autocommit_or_mark_failed(&mut session, started_txn).await;
                    drop(session);
            ...
            statement_dirty_tables.extend(chunk_dirty_tables);
            offset = end;
        }
        ...
        PgWireError: From<<C as Sink<PgWireBackendMessage>>::Error>,
    {
        ...
        let (row_count, started_txn, dirty_table_ids) = if let Some(mut ctx) = ctx_opt {
            ...
            let (row_count, statement_dirty_tables) =
                    ...
                    if let Some(final_line_bytes) = ctx.drain_final_line() {
                        if final_line_bytes.as_slice() == b"\\." {
                            ...
                            let col_values = match parse_copy_input_line(
                                ...
                                Ok(values) => values,
                                Err(e) => {
                                    rollback_autocommit_or_mark_failed(
                                        &mut session,
                                        ctx.started_txn,
                                    )
                                    .await;
                            ...
                            let mut final_deferred =
                                std::mem::take(&mut ctx.deferred_self_fk_checks);
...
mod tests {
    ...
    async fn copy_statement_context_provides_full_statement_runtime_context() {
        ...
        let result = with_copy_statement_context(&qctx, &runtime, async {
            ...
            assert_eq!(
                &*crate::session_context::current_txn_dirty_table_ids(),
                &HashSet::from([9_u64])
            );
            assert_eq!(
    ...
    async fn copy_statement_context_reuses_statement_extension_state_across_reentry() {
        ...
        let second = with_copy_statement_context(&qctx, &runtime, async {
            crate::extensions::context::try_consume_embedding_call_with_limit(1)
    ...
    #[tokio::test]
    async fn copy_statement_context_returns_statement_dirty_tables() {
        let qctx = QueryContext::for_tests();
        let runtime = StatementRuntimeContext {
            settings: RuntimeSettings {
                is_superuser: false,
        ...
        let result = with_copy_statement_context(&qctx, &runtime, async {
            crate::session_context::record_statement_dirty_table_id(42);
            Ok(())
        })
        .await;
