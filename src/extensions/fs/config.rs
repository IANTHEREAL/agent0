// Embedded/PageFS + S3 fs9 configuration: no longer consumed by production routing
...
// -D warnings` stays green without deleting config we intend to revive.
#![allow(dead_code)]
...
pub(crate) fn fs9_config() -> &'static Fs9Config {
    FS9_CONFIG.get_or_init(|| Fs9Config::from_env().expect("fs9 configuration error"))
}
