...
//!    `scp` covering the requested mode. db9-server obtains the token from
//!    db9-backend's `/internal/connect-token/exchange` endpoint (see
//!    `auth::fs_plane_token`); it is NEVER an issuer.
