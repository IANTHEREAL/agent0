...
use crate::auth::fs_plane_token::{ExchangeConfig, Fs9PlaneTokenCache};
use crate::extensions::fs::backend::{
    FsBackend, FsBatchWriteEntry, FsBatchWriteFile, FsCreateUpload, FsFileInfo,
    FsMultipartCompletedPart, FsPreparedDownload, FsPresignedRequest, FsWriteStream,
...
/// production wiring is the `ExchangeTokenProvider` below; tests inject
/// a static one.
#[async_trait]
...
/// Production provider: hits db9-backend's `/internal/connect-token/exchange`
/// via the process-wide cache.
pub(crate) struct ExchangeTokenProvider {
...
impl GrpcFsBackend {
    pub(crate) fn new(
        ...
        let volume_id = format!("jfs_t_{tenant_id}");
        Self {
            channel,
            volume_id,
            token_provider,
    ...
    fn client(&self) -> FsPlaneClient<Channel> {
        FsPlaneClient::new(self.channel.clone())
    }
...
impl GrpcMultipartUpload {
    async fn begin(
        ...
        .await?;
        let resp: BeginWriteResponse = FsPlaneClient::new(channel.clone())
            .begin_write(req)
            .await
    ...
    async fn put_all_chunks(&mut self, chunks: Vec<Vec<u8>>) -> Result<()> {
        ...
        let resp: WritePartsResponse = FsPlaneClient::new(self.channel.clone())
            .write_parts(req)
            .await
    ...
    async fn open_stream(&self) -> Result<GrpcMultipartUploadStream> {
        ...
        let task: JoinHandle<Result<u64>> = tokio::spawn(async move {
            let resp: WritePartsResponse = FsPlaneClient::new(channel)
                .write_parts(req)
                .await
    ...
    async fn commit(mut self) -> Result<u64> {
        ...
        .await?;
        let resp: CommitWriteResponse = FsPlaneClient::new(self.channel.clone())
            .commit_write(req)
            .await
    ...
    async fn abort(mut self) -> Result<()> {
        ...
        .await?;
        let resp: AbortWriteResponse = FsPlaneClient::new(self.channel.clone())
            .abort_write(req)
            .await
        ...
        match tokio::runtime::Handle::try_current() {
            Ok(handle) => {
                handle.spawn(async move {
                    ...
                    let _ = FsPlaneClient::new(channel).abort_write(req).await;
                });
            }
            Err(_) => {
                warn!(
...
impl FsWriteStream for GrpcWriteStream {
    ...
    async fn terminate(mut self: Box<Self>, outcome: Result<()>) -> Result<usize> {
        ...
        match outcome {
            ...
            Ok(()) => {
                // Flush any staged sub-chunk tail, then close stream, then
                // commit.
                if !self.buf.is_empty() {
                ...
                let bytes = match self.stream.take() {
                    ...
                    None => return Err(anyhow!("gRPC write stream missing on terminate(Ok)")),
                };
...
impl FsBackend for GrpcFsBackend {
    ...
    async fn read_file(&self, path: &str, max_bytes: usize) -> Result<Vec<u8>> {
        ...
        // Embedded backend's `read_file` errors when the file exceeds
        // `max_bytes` (see `embedded/pagefs/read_impl.rs:503-508`).
        // Mirror that contract here so SQL `fs9_read`, `COPY FROM
        // fs9://`, and parquet readers can't be fooled into parsing a
        // silent prefix as if it were the whole file.
    ...
    async fn read_file_stream(
        ...
        // Same contract as `read_file`: embedded errors before exposing
        // any bytes when the file exceeds the caller's cap
        // (`embedded/pagefs/read_impl.rs:780-784`). CSV / TSV / JSON
        // line-by-line decoders downstream (`fs/file_stream.rs:69`,
        // `fs/glob_stream.rs:186`, `sql/executor/extensions.rs:747`)
        // expect a sanity cap to mean "full file or error" — silently
        // streaming the first max_bytes would parse a prefix as
        // complete data.
        let info = self.stat(path).await?;
        if info.size > max_bytes as u64 {
            return Err(anyhow!(EmbeddedFsError::TooLarge(format!(
            ...
            match frame.frame {
                Some(read_at_response::Frame::Chunk(chunk)) => {
                    if data.len().saturating_add(chunk.data.len()) > length {
                        // Server may legitimately return more than the
                        // requested length (chunk granularity). Truncate
                        // to match the caller's cap so downstream
                        // consumers see exactly what they asked for.
                        let want = length - data.len();
                        data.extend_from_slice(&chunk.data[..want]);
                        // Drop the rest by exhausting the stream — the
                        // server still has bytes in flight; cancelling
                        // mid-stream sends a RST which the server logs
                        // as a client cancel. Acceptable.
                        return Ok(data);
                    }
