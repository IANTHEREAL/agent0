...
fn distinct_order_by_value_arg_len(func_name: &str, arg_len: usize) -> usize {
    if matches!(func_name, "STRING_AGG" | "BYTEA_STRING_AGG") {
        arg_len.min(1)
    } else {
        arg_len
    }
...
fn strip_noop_cast(expr: &TypedExpr) -> &TypedExpr {
    ...
    loop {
        match &current.kind {
            TypedExprKind::Cast {
                ...
                target_type,
                ..
            } if *target_type == inner.data_type && current.data_type == inner.data_type => {
                current = inner;
            }
            _ => return current,
        }
...
fn typed_expr_eq(a: &TypedExpr, b: &TypedExpr) -> bool {
    let a = strip_noop_cast(a);
    let b = strip_noop_cast(b);
...
fn typed_expr_kind_eq(a: &TypedExprKind, b: &TypedExprKind) -> bool {
    ...
    match (a, b) {
        ...
        ) => na == nb && typed_expr_eq(la, lb) && typed_expr_eq(ra, rb),
        (
            Between {
                expr: ea,
                low: la,
        ...
        (
            ...
            cia == cib
                ...
                && typed_expr_eq(ea, eb)
                && typed_expr_eq(pa, pb)
                && opt_typed_expr_eq(xa.as_deref(), xb.as_deref())
        }
        (
            SimilarTo {
                expr: ea,
        ...
        (
            ...
            fa.name == fb.name
                ...
                && oa.len() == ob.len()
                && aa.iter().zip(ab.iter()).all(|(x, y)| typed_expr_eq(x, y))
                && pa.iter().zip(pb.iter()).all(|(x, y)| typed_expr_eq(x, y))
                && oa
                    .iter()
                    .zip(ob.iter())
                    .all(|(x, y)| typed_order_by_expr_eq(x, y))
                && opt_window_frame_eq(wa.as_ref(), wb.as_ref())
        }
        (ScalarSubquery(qa), ScalarSubquery(qb)) => format!("{qa:?}") == format!("{qb:?}"),
        (
            Exists {
                subquery: qa,
                negated: na,
        ...
        ) => na == nb && typed_expr_eq(ea, eb) && format!("{qa:?}") == format!("{qb:?}"),
        (
            TupleInSubquery {
                exprs: ea,
                subquery: qa,
            ...
            na == nb
                && ea.len() == eb.len()
                && ea.iter().zip(eb.iter()).all(|(x, y)| typed_expr_eq(x, y))
                && format!("{qa:?}") == format!("{qb:?}")
        }
        (
            AnyAll {
            ...
            oa == ob && ia == ib && typed_expr_eq(ea, eb) && format!("{qa:?}") == format!("{qb:?}")
        }
        (ArraySubquery(qa), ArraySubquery(qb)) => format!("{qa:?}") == format!("{qb:?}"),
        (ArrayLiteral(aa), ArrayLiteral(ab)) => {
            aa.len() == ab.len() && aa.iter().zip(ab.iter()).all(|(x, y)| typed_expr_eq(x, y))
        }
        (
            ArrayIndex {
        ...
        ) => ca == cb && ra == rb && typed_expr_eq(ea, eb),
        _ => false,
    }
}
...
fn typed_order_by_expr_eq(a: &TypedOrderByExpr, b: &TypedOrderByExpr) -> bool {
    a.asc == b.asc && a.nulls_first == b.nulls_first && typed_expr_eq(&a.expr, &b.expr)
}

fn opt_window_frame_eq(a: Option<&WindowFrame>, b: Option<&WindowFrame>) -> bool {
    match (a, b) {
        (None, None) => true,
        (Some(x), Some(y)) => window_frame_eq(x, y),
        _ => false,
    }
}

fn window_frame_eq(a: &WindowFrame, b: &WindowFrame) -> bool {
    if a.units != b.units {
        return false;
    }
    if !window_frame_bound_eq(&a.start, &b.start) {
        return false;
    }
    match (&a.end, &b.end) {
        (None, None) => true,
        (Some(x), Some(y)) => window_frame_bound_eq(x, y),
        _ => false,
    }
}

fn window_frame_bound_eq(a: &WindowFrameBound, b: &WindowFrameBound) -> bool {
    match (a, b) {
        (WindowFrameBound::CurrentRow, WindowFrameBound::CurrentRow) => true,
        (WindowFrameBound::Preceding(ae), WindowFrameBound::Preceding(be))
        | (WindowFrameBound::Following(ae), WindowFrameBound::Following(be)) => {
            match (ae.as_deref(), be.as_deref()) {
                (None, None) => true,
                (Some(x), Some(y)) => typed_expr_eq(x, y),
                _ => false,
            }
    ...
    pub(super) fn analyze_function(&mut self, func: &Function) -> Result<TypedExpr, AnalyzerError> {
        ...
        if let Some(sig) = registry.get(&func_name) {
            ...
            if sig.is_aggregate {
                ...
                if func.distinct && !order_by.is_empty() {
                    let value_arg_len =
                        distinct_order_by_value_arg_len(&func_name, analyzed_args.len());
                    for ob in &order_by {
                        if !analyzed_args
                            .iter()
                            .take(value_arg_len)
                            .any(|arg| typed_expr_eq(&ob.expr, arg))
                        {
                            return Err(AnalyzerError::InvalidColumnReference(
