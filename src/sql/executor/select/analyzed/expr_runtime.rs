...
impl<'a> ExprRuntime<'a> {
    ...
    /// Outer-join-aware async ON post-filter.
    ///
    /// Unlike simple WHERE filtering (which drops non-matching rows), this
    /// preserves null-extension semantics: if all matched rows for a given
    /// outer-side key fail the async predicate, one null-extended row is emitted.
    pub async fn filter_async_outer_join(
        &self,
        rows: Vec<Row>,
        info: &super::OuterJoinAsyncInfo,
        schema: &TableSchema,
        txn: &mut Transaction,
        seq: &mut HashMap<String, i64>,
    ) -> Result<Vec<Row>> {
        ...
        let right_start = info.right_col_start;
        let right_end = right_start + info.right_col_count;
        let total_cols = rows[0].values.len();

        match info.join_type {
            JoinType::Left => {
                self.filter_left_join(
                    rows,
                    &info.async_pred,
                    schema,
                    txn,
            ...
            _ => {
                // Inner/Cross should not reach here (extracted to WHERE instead).
                Ok(rows)
            }
        }
    }

    /// LEFT JOIN async ON filter: group by left-side values. For each group,
    /// null-extended rows pass through; matched rows are evaluated against the
    /// async predicate. If no matched row survives, emit one null-extended row.
    async fn filter_left_join(
        &self,
        rows: Vec<Row>,
        pred: &TypedExpr,
        schema: &TableSchema,
        ...
        // Group consecutive rows by left-side key identity.
        // The NLJ emits rows grouped by the outer (left) side, so we can
        // process groups in order without needing a hash map.
        let mut result = Vec::with_capacity(rows.len());
        let mut i = 0;
        while i < rows.len() {
            // Collect all rows that share the same left-side values.
            let left_key: Vec<&Value> = rows[i]
                .values
                .iter()
                .enumerate()
                .filter(|(idx, _)| *idx < right_start || *idx >= right_end)
                .map(|(_, v)| v)
                .collect();
            ...
            for row in &rows[group_start..i] {
                // Check if this is already a null-extended row (right side all NULLs).
                let right_all_null = row.values[right_start..right_end]
                    .iter()
                    .all(|v| matches!(v, Value::Null));
                if right_all_null {
                    // Already null-extended from NLJ (no sync-ON match) → keep.
                    result.push(row.clone());
                    any_survived = true;
                } else {
                    // Matched row — evaluate async predicate.
                    let materialized = self
                        .executor
                        .materialize_expr_for_row(
                            pred,
            ...
            // If no row survived for this left key, emit null-extended.
            if !any_survived {
                let mut null_row = rows[group_start].clone();
                for idx in right_start..right_end {
                    if idx < null_row.values.len() {
                        null_row.values[idx] = Value::Null;
                    }
                }
    ...
    /// RIGHT JOIN async ON filter: mirror of LEFT — group by right-side values.
    async fn filter_right_join(
        &self,
        rows: Vec<Row>,
        pred: &TypedExpr,
        schema: &TableSchema,
        ...
        // For RIGHT JOIN the NLJ iterates right as outer, so rows are grouped
        // by right-side values. The left side gets null-extended.
        let mut result = Vec::with_capacity(rows.len());
        let mut i = 0;
        while i < rows.len() {
            let right_key: Vec<&Value> = rows[i].values[right_start..right_end].iter().collect();

            let group_start = i;
            i += 1;
            while i < rows.len() {
                let cur_key: Vec<&Value> = rows[i].values[right_start..right_end].iter().collect();
                if cur_key == right_key {
                    i += 1;
                } else {
                    break;
                }
            ...
            let mut any_survived = false;
            for row in &rows[group_start..i] {
                // Left side all NULLs means already null-extended.
                let left_all_null = row
                    .values
                    .iter()
                    .enumerate()
                    .filter(|(idx, _)| *idx < right_start || *idx >= right_end)
            ...
            if !any_survived {
                let mut null_row = rows[group_start].clone();
                // Null-extend the left side (everything outside right range).
    ...
    /// FULL JOIN async ON filter: combines LEFT and RIGHT logic.
    async fn filter_full_join(
        &self,
        rows: Vec<Row>,
        pred: &TypedExpr,
        schema: &TableSchema,
        ...
        // For FULL JOIN, we need to track both sides. The NLJ processes left
        // as outer, so rows are grouped by left-side values first.
        // Pass 1: Apply LEFT join logic (preserve left side, null-extend right).
        let after_left = self
            .filter_left_join(rows, pred, schema, txn, seq, right_start, right_end)
            .await?;

        // Pass 2: For rows that were null-extended on the right side, also check
        // if the right side needs its own null-extended rows (right rows that
        // didn't match any left row). These are already emitted by the NLJ as
        // (NULLs, right) rows. We keep them as-is (they're already null-extended
        // on the left side).
        // The LEFT pass already preserves (NULLs, right) rows and handles
        // (left, right) → (left, NULLs) downgrade correctly.
        Ok(after_left)
    }
}
