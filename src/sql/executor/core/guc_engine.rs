...
pub(crate) fn check_reserved_guc_write(name: &str) -> Result<()> {
    match classify_guc(name) {
        ...
        .into()),
        GucKind::SessionAuthPseudo => Err(SqlError::InvalidParameterValue {
...
#[cfg(test)]
mod tests {
    use super::{check_reserved_guc_reset, check_reserved_guc_write, classify_guc, GucKind};
    ...
    fn reserved_pseudo_write_and_reset_rules_match_pg_compat() {
        let err = check_reserved_guc_write("is_superuser")
            .unwrap_err()
            .to_string();
        assert!(err.contains("parameter \"is_superuser\" cannot be changed"));

        let err = check_reserved_guc_write("session_authorization")
            .unwrap_err()
            .to_string();
