...
fn decorrelate_children(plan: LogicalPlan) -> LogicalPlan {
    ...
    let node = match plan.node {
        LogicalNode::Filter { predicate, input } => LogicalNode::Filter {
            predicate,
            input: Box::new(decorrelate_recursive(*input)),
        },
        ...
        LogicalNode::Join {
            ...
            left: Box::new(decorrelate_recursive(*left)),
            right: Box::new(decorrelate_recursive(*right)),
            join_type,
            condition,
        },
        ...
        LogicalNode::Subquery { subplan, alias } => LogicalNode::Subquery {
            subplan: Box::new(decorrelate_recursive(*subplan)),
            alias,
        },
        // Leaf nodes
