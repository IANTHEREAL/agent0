...
// ---------------------------------------------------------------------------
// Extracted helpers (one per ALTER TABLE sub-operation)
// ---------------------------------------------------------------------------
...
/// constraint name.
async fn alter_table_add_primary_key(
    store: &Arc<TikvStore>,
    txn: &mut Transaction,
    db_id: u64,
    schema: &mut crate::types::TableSchema,
...
async fn alter_table_add_unique_constraint(
    store: &Arc<TikvStore>,
    txn: &mut Transaction,
    db_id: u64,
    schema: &mut crate::types::TableSchema,
...
/// check constraint.
async fn alter_table_add_check_constraint(
    store: &Arc<TikvStore>,
    txn: &mut Transaction,
    db_id: u64,
    search_path: &[String],
...
/// be invalidated.
async fn alter_table_alter_column_set_data_type(
    store: &Arc<TikvStore>,
    txn: &mut Transaction,
    db_id: u64,
    search_path: &[String],
