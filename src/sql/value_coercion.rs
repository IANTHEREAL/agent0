...
pub fn parse_pg_array(s: &str) -> Result<Vec<Value>> {
        ...
        match c {
            ...
            '}' if !in_quotes => {
                nested_depth = nested_depth
                    .checked_sub(1)
                    .ok_or_else(|| anyhow!("Invalid array format"))?;
                current.push(c);
            }
            ',' if !in_quotes && nested_depth == 0 => {
    ...
    if in_quotes || escape_next || nested_depth != 0 {
        return Err(anyhow!("Invalid array format"));
    }

    if !current.is_empty() || inner.ends_with(',') || quoted_element {
        let val = parse_array_element(&current, quoted_element)?;
        result.push(val);
    }
...
fn parse_array_element(s: &str, quoted: bool) -> Result<Value> {
    if quoted {
        return Ok(Value::Text(s.to_string()));
    }

    let s = s.trim();
    if !quoted && s.starts_with('{') && s.ends_with('}') {
        return parse_pg_array(s).map(Value::Array);
    }
...
pub fn parse_value_for_copy(val: &str, data_type: &DataType) -> Result<Value> {
    ...
    match data_type {
        ...
        DataType::Unknown => Ok(Value::Text(unescaped)),
    }
}
...
mod tests {
    ...
    #[test]
    fn test_parse_pg_array_quoted_empty_string_is_text() {
        let result = parse_pg_array("{\"\"}").unwrap();
        assert_eq!(result, vec![Value::Text(String::new())]);
    }

    #[test]
    fn test_parse_pg_array_preserves_quoted_text_lexemes() {
        let result = parse_pg_array("{\" 01 \",\"true\",\"NULL\",\"a,b\"}").unwrap();
        assert_eq!(
            result,
            vec![
