...
| `DB9_GC_LIFE_TIME_SEC` | `86400` | `src/worker/config.rs`, `src/worker/gc.rs` | MVCC retention window for time-based safepoint calculation. Active transactions are protected by direct registry tracking, not by this timeout. |
