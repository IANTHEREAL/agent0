...
--   - DELETE USING with subquery wrapper executes successfully in PG (DELETE 5).
...
-- DELETE USING with subquery wrapper exceeds limit.
DELETE FROM dml_err_t1 USING (SELECT * FROM dml_err_t2) s WHERE dml_err_t1.id = s.id;
