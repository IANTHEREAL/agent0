...
--   - `UPDATE ... FROM cp_aux_a, cp_aux_b` executes and updates 1 row.
-- db9-specific: db9 adds a combined auxiliary cross-product cap and errors here.
-- Note: UPDATE path is parser-rewritten to CROSS JOIN and trips the JOIN output guard.
...
-- UPDATE FROM with two auxiliary sources.
UPDATE cp_target
SET val = cp_aux_a.val
FROM cp_aux_a, cp_aux_b
