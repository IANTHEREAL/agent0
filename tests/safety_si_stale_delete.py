#!/usr/bin/env python3
"""
Snapshot-isolation stale DELETE guard.

Txn A starts and reads a row. Txn B commits a newer version while A is still
open. When A later tries to DELETE that snapshot row, db9 must return 40001
instead of deleting the newer committed row.
"""
...
def main():
    parser = argparse.ArgumentParser(description="SI stale DELETE guard")
    parser.add_argument(
        "--dsn",
    ...
    table = f"si_stale_delete_{int(time.time())}_{random.randint(1000, 9999)}"
    ...
    err = txn_a["error"]
    if err is None:
        print("FAIL: stale DELETE succeeded; expected SQLSTATE 40001")
        sys.exit(1)
    if err.sqlstate != "40001":
        print(f"FAIL: stale DELETE failed with SQLSTATE {err.sqlstate}, expected 40001")
        print(err)
        sys.exit(1)
