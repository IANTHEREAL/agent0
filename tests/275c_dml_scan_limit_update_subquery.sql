...
--   - UPDATE FROM with subquery wrapper executes successfully in PG (UPDATE 5).
...
-- UPDATE FROM with subquery wrapper exceeds limit.
UPDATE dml_err_t1 SET val = s.val FROM (SELECT * FROM dml_err_t2) s WHERE dml_err_t1.id = s.id;
