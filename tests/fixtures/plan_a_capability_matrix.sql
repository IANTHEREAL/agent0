-- =========================
-- Plan A Capability Matrix (p_-prefixed diagnostic variant)
--
-- Purpose: run against current master to identify which features work
-- and which are blocked. Parameters renamed with p_ prefix to avoid
-- text-substitution collision with column names.
--
-- NOT the acceptance target — the original Plan A SQL (same-name params)
