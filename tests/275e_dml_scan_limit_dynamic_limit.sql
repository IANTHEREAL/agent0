...
--   - Dynamic LIMIT via prepared statement executes successfully in PG (UPDATE 5).
...
-- Dynamic LIMIT over cap still fails.
PREPARE dml_err_p1(int) AS
UPDATE dml_err_t1
SET val = s.val
FROM (SELECT id, val FROM dml_err_t2 LIMIT $1) s
WHERE dml_err_t1.id = s.id;
EXECUTE dml_err_p1(5);
DEALLOCATE dml_err_p1;
