...
FROM extensions.fs9('/tmp/db9-fs9-test/sales/*.csv', format => 'csv', header => true, exclude => '*jan.csv')
ORDER BY _path, product;

SELECT *
FROM extensions.fs9('/tmp/db9-fs9-test/sales/*.csv', format => 'csv', header => true, exclude => '*jan.csv,*feb.csv');
