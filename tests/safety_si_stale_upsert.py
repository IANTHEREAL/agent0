#!/usr/bin/env python3
"""
Snapshot-isolation stale UPSERT guard.

Txn A starts and reads a row. Txn B commits a newer version while A is still
open. When A later runs INSERT ... ON CONFLICT DO UPDATE against that row, db9
must return 40001 instead of updating from A's stale snapshot.
"""
...
def main():
    parser = argparse.ArgumentParser(description="SI stale UPSERT guard")
    parser.add_argument(
        "--dsn",
    ...
    table = f"si_stale_upsert_{int(time.time())}_{random.randint(1000, 9999)}"
        ...
        def run():
            with psycopg.connect(args.dsn, autocommit=True) as conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        f"""
                        INSERT INTO {table}(id, v) VALUES (1, 20)
    ...
    err = txn_a["error"]
    if err is None:
        print("FAIL: stale UPSERT succeeded; expected SQLSTATE 40001")
        sys.exit(1)
    if err.sqlstate != "40001":
        print(f"FAIL: stale UPSERT failed with SQLSTATE {err.sqlstate}, expected 40001")
        print(err)
        sys.exit(1)
